...
class TripsListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isUpcoming ? 'Upcoming Adventures' : 'Past Journeys', style: const TextStyle(fontWeight: FontWeight.bold)),
      ),
      body: filteredTrips.isEmpty
          ? Center(child: Text('No journeys to show here.', style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.3))))
          : ListView.separated(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                             ...
                             const Gap(4),
                             Text(trip.destination, style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold, fontSize: 12)),
                          ],
                        ),
                      ],
                    ),
                  ...
                  Column(
                    ...
                    children: [
                       Text(
                         ...
                         style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.5), fontWeight: FontWeight.bold, fontSize: 12),
                       ),
                       const Gap(4),
                       Text('${trip.totalBudget.toStringAsFixed(0)} Budget', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ],
              ),
